...
RegisterNetEvent('HT_maria:UseSeed')
RegisterNetEvent('HT_maria:UseItem')
RegisterNetEvent('HT_maria:UseBag')
RegisterNetEvent('HT_maria:UseRollingPapers')
...
function HTM:Start()
  ...
  ESX.TriggerServerCallback('HT_maria:GetLoginData', function(plants)
...
function HTM:GetLoginData()
end

function HTM:PerSecThread()
  while true do
    Wait(1000)
...
function HTM:TextHandler()
  ...
  if closest.Growth >= 99.99 then
    ...
    if closest.Owner == plyId  then
      strA = strA .. "[ Presiona [ E ] para recolectar ] "
  ...
  if (plyData.job and plyData.job.label and plyData.job.label == self.PoliceJobLabel) then
    self.PolText = { closest = closest, strA = "[ Presiona [~r~ E ~s~] para incautar ]" }
  end
  self.CurKey = closestKey
end

function HTM:GetValColour(v)
...
function HTM:GrowthHandlerSlow()
  if not self.Plants then return; end
  if not #self.Plants then return; end
  local plyData = ESX.GetPlayerData()
  local plyId = plyData.identifier
...
function HTM:GrowPlantSlow(plant,key)
  if not self.Plants then return; end
...
function HTM:GrowthHandlerFast()
  if not self.Plants then return; end
  if not #self.Plants then return; end
  local plyData = ESX.GetPlayerData()
  local plyId = plyData.identifier
...
function HTM:GrowPlantFast(plant,key)
  if not self.Plants then return; end
  if not self.Plants[key] then return; end
  if self.Plants[key] ~= plant then return; end
...
function HTM:EnterInstance(instance)
  self.Instance = instance.data.owner
  print("Enter Instance : HTM")
  if not self.Plants then return; end
  if not #self.Plants then return; end
  for k = 1,#self.Plants,1 do
    local v = self.Plants[k]
    if v and v.Instance then
      if v.Instance == self.Instance then
        HTM:SpawnInstance(v,k)
      end
    end
  end
end

function HTM:LeaveInstance(instance)
  print("Leave Instance : HTM")
  if not self.Plants then return; end
  if not #self.Plants then return; end
  self.Instance = false
  for k = 1,#self.Plants,1 do
...
function HTM:SpawnInstance(plant,k)
...
function HTM:SpawnWorld(plant,k)
...
function HTM:SyncHandler(plant,delete)
...
function HTM:Awake(...)
    ...
    ESX.TriggerServerCallback('HT_maria:GetStartData', function(retVal) self.dS = true; self.cS = retVal; end)
    while not self.dS do Citizen.Wait(0); end
    self:Start()
end

function HTM:GetPlantZ(plant)
  if plant.Stage <= 3 then return -1.0
  else return -3.5
  end
end

function HTM:UseItem(item)
  if not self.Plants then return; end
  if not #self.Plants then return; end
...
function HTM:Sync(plant,delete)
  TriggerServerEvent('HT_maria:SyncPlant',plant,delete)
end

function HTM:UseBag(canUse, msg)
  Citizen.CreateThread(function(...)
    local plyPed = GetPlayerPed(-1)
...
function HTM:DrawText3D(x,y,z, text)
...
AddEventHandler('instance:onEnter', function(instance) while not HTM.iR do Citizen.Wait(0); end HTM:EnterInstance(instance); end)
AddEventHandler('instance:onLeave', function(instance) if HTM.iR then HTM:LeaveInstance(instance); else HTM.Instance = false; end; end)
AddEventHandler('HT_maria:UseSeed', function(seed) if HTM.iR then HTM:UseSeed(seed); end; end)
AddEventHandler('HT_maria:UseItem', function(item) if HTM.iR then HTM:UseItem(item); end; end)
AddEventHandler('HT_maria:SyncPlant', function(plant,del) if HTM.iR then HTM:SyncHandler(plant,del); end; end)
AddEventHandler('HT_maria:UseBag', function(canUse,msg) HTM:UseBag(canUse,msg); end)
